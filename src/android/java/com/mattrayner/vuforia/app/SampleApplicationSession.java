...
public class SampleApplicationSession implements UpdateCallbackInterface
{
    ...
    private String getInitializationErrorString(int code)
    {
        if (code == INIT_ERRORCODE.INIT_DEVICE_NOT_SUPPORTED)
            return "Failed to initialize Vuforia Engine. This device is not supported.";
        if (code == INIT_ERRORCODE.INIT_NO_CAMERA_ACCESS)
            return "Failed to initialize Vuforia Engine. Camera not accessible";
        if (code == INIT_ERRORCODE.INIT_LICENSE_ERROR_MISSING_KEY)
            return "Vuforia Engine App key is missing. Please get a valid key, by logging into your account at developer.vuforia.com and creating a new project.";
        if (code == INIT_ERRORCODE.INIT_LICENSE_ERROR_INVALID_KEY)
            return "Invalid Key used. Please make sure you are using a valid Vuforia Engine App Key.";
        ...
        if (code == INIT_ERRORCODE.INIT_LICENSE_ERROR_PRODUCT_TYPE_MISMATCH)
            return "Vuforia Engine App key is not valid for this product. Please get a valid key, by logging into your account at developer.vuforia.com and choosing the right product type during project creation.";
        else
        {
            return "Failed to initialize Vuforia Engine.";
        }
    }
